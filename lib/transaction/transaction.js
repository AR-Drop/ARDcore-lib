...
Transaction.MAX_MONEY = 100000000000 * 1e8;
...
/**
 * Retrieve a hexa string that can be used with ARDd's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more ARDs than the sum of the input amounts
 * @return {string}
 */
 ...
 *
 * Can receive, as output information, the output of ARDd's `listunspent` command,
 * and a slightly fancier format recognized by ARDcore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a ARDcore Address object. The
 * same is true for `script`, which can be a string or a ARDcore Script.
 *
 ...
 *
 * // From a pay to public key hash output from ARDd's listunspent
...
/**
 * Analogous to ARDd's IsCoinBase function in transaction.h
 */
